...
private static String getProperty(final String prop) {
    ...
    int getTexture2DId() {
    	...
    	MemoryFreeList textureIds = FreeListManager.getFreeList(FreeListManager.TEXTURE2D);
		int id;	
		synchronized (textureIdLock) {
		    if (textureIds.size() > 0) {
		    	id = ((Integer)FreeListManager.getObject(FreeListManager.TEXTURE2D)).intValue();
		    } 
		    else {
		    	id = (++textureIdCount);
		    }
    ...
    int getTexture3DId() {
    	// MasterControl has to handle the ID itself.  2D and 3D ideas must
    	// never be the same, so the counter has to be in the MasterControl
    	MemoryFreeList textureIds = FreeListManager.getFreeList(FreeListManager.TEXTURE3D);
		synchronized (textureIdLock) {
		    if (textureIds.size > 0) {
		    	return ((Integer)FreeListManager.getObject(FreeListManager.TEXTURE3D)).intValue();
		    }
		    else 
